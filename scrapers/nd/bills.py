...
class NDBillScraper(Scraper, LXMLMixin):
    ...
    house_list_url = "http://www.legis.nd.gov/assembly/%s-%s/%sbill-text/house-bill.html"
    senate_list_url = (
        ...
        for item in self.jurisdiction.legislative_sessions:
            if item["identifier"] == session:
                ...
                if 'classification' in item and item["classification"] == "special":
                    ...
                    print(self.url_session)
                break
        # Get the subjects for every bill
        # Sometimes, at least with prefiles, a bill will not be given subjects
        self.subjects_url = self.subjects_url % (self.url_session, start_year, self.special_url_slug)
        self.subjects = {}
        self.scrape_subjects(session)

        for chamber, url in {
            "lower": self.house_list_url % (self.url_session, start_year, self.special_url_slug),
            "upper": self.senate_list_url % (self.url_session, start_year, self.special_url_slug),
        }.items():
