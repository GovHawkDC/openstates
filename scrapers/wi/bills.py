...
class WIBillScraper(Scraper):
    ...
    def scrape_subjects(self, year, site_id):
        ...
        while last_url != next_url:
            html = self.get(next_url, headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36"}).text
    ...
    def scrape_bill_list(self, chamber, session, url):
        ...
        try:
            data = self.get(url, headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36"}).text
    ...
    def scrape_bill_history(self, bill, url, chamber):
        seen_votes = set()
        body = self.get(url, headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36"}).text
        ...
        for a in doc.xpath('//ul[@class="docLinks"]/li//a'):
            ...
            if not a.text:
                ...
                extra_doc = lxml.html.fromstring(self.get(extra_doc_url, headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36"}).text)
    ...
    def add_senate_votes(self, vote, url):
        try:
            html = self.get(url, headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36"}).text
        except scrapelib.HTTPError:
    ...
    def add_house_votes(self, vote, url):
        try:
            html = self.get(url, headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36"}).content
        except scrapelib.HTTPError:
