...
class VTPersonScraper(Scraper, LXMLMixin):
    ...
    def scrape(self, session=None):
        ...
        legislator_dump_url = "http://legislature.vermont.gov/people/loadAll/{}".format(year_slug)
        ...
        for info in legislators:
            ...
            member_url = "http://legislature.vermont.gov/people/single/{}/{}".format(year_slug, info["PersonID"])
            ...
            state_email = page.xpath(
                '//dl[@class="summary-table profile-summary"]/' 'dt[text()="Email"]/following-sibling::dd[1]/a/text()'
            )
            if state_email:
            ...
            leg.add_contact_detail(
                ...
                value="{0}{1}\n{2}, {3} {4}".format(
                    ...
                    ("\n" + info["MailingAddress2"] if info["MailingAddress2"].strip() else ""),
                    info["MailingCity"],
                    info["MailingState"],
                    info["MailingZIP"],
                ),
