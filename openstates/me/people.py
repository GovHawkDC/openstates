...
class MEPersonScraper(Scraper):
    ...
    def scrape_rep(self, url):
        ...
        district_number = page.xpath('//span[contains(text(), "House District:")]' "/following-sibling::span/text()")[
            0
        ].strip()
        ...
        person = Person(name=name, district=district_number, primary_org="lower", party=party, image=photo_url,)
    ...
    def scrape_senator(self, district):
        ...
        title_match = re.match(r"District (\d+) - State Senator ([^\(]+) \(([DRI])", title)
        ...
        person = Person(name=name, district=district, image=image_url, primary_org="upper", party=party,)
        ...
        if phone:
            person.add_contact_detail(type="voice", value=clean_phone(phone), note="District Phone")
