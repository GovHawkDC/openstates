...
class NMBillScraper(Scraper):
    def _init_mdb(self, session):
        ...
        fname_re = r"(\d{{2}}-\d{{2}}-\d{{2}}  \d{{2}}:\d{{2}}(?:A|P)M) .* " "({fname}.*zip)".format(fname=fname)

        # use listing to get latest modified LegInfo zip
        listing = self.get(ftp_base).text
        matches = re.findall(fname_re, listing)
        matches = sorted([(datetime.strptime(date, "%m-%d-%y  %H:%M%p"), filename) for date, filename in matches])
        if not matches:
    ...
    def scrape_chamber(self, chamber, session):
        ...
        bills = {}
        for data in [row for row in self.access_to_csv("Legislation") if row["BillID"].startswith(chamber_letter)]:
            # use their BillID for the key but build our own for storage
            bill_key = data["BillID"].replace(" ", "")

            # remove spaces for consistency
            bill_id = "{}{}{}".format(data["Chamber"], data["LegType"], data["LegNo"]).replace(" ", "")
            bill_type = bill_type_map[data["LegType"]]
            bills[bill_key] = bill = Bill(
                bill_id, legislative_session=session, chamber=chamber, title=data["Title"], classification=bill_type,
            )

            # fake a source
            data["SessionYear"] = session_year
            data.update({x: data[x].strip() for x in ["Chamber", "LegType", "LegNo", "SessionYear"]})
            ...
            bill.add_sponsorship(
                sponsor_map[data["SponsorCode"]], classification="primary", entity_type="person", primary=True,
            )
            for sponsor_code in [
                "SponsorCode2",
                "SponsorCode3",
                ...
                if data[sponsor_code] and data[sponsor_code] not in ("NONE", "X", ""):
                    bill.add_sponsorship(
                        sponsor_map[data[sponsor_code]], classification="primary", entity_type="person", primary=True,
                    )
                    ...
                    if (chamber == "upper" and bill_type[0] == "S") or (chamber == "lower" and bill_type[0] == "H"):
                        ...
                        try:
                            ...
                            if doc_type == "Final Version":
                                bill.add_version_link("Final Version", url + fname, media_type=mimetype)
                            elif doc_type == "Amendments in Context":
                                bill.add_version_link(doc_type, url + fname, media_type=mimetype)
                            else:
                                bill.add_document_link(
                                    doc_type, url + fname, media_type=mimetype, on_duplicate="ignore",
                                )
    ...
    def scrape_actions(self, chamber_letter, bills):
        ...
        action_map = {
            ...
            "7624": (
                "WITHOUT RECOMMENDATION, as amended, committee report adopted, placed " "on Speaker's table",
                "committee-passage",
            ),
            "7625": (
            ...
            "7655": ("Referred to the House Appropriations & Finance", "referral-committee",),
        ...
        for action in [row for row in self.access_to_csv("Actions") if row["BillID"].startswith(chamber_letter)]:
            ...
            action_date = datetime.strptime(action["EntryDate"].split()[0], "%m/%d/%y").strftime("%Y-%m-%d")
            ...
            try:
                ...
                self.warning("unknown action code {} on {}".format(action_code, bill_key))
                raise
            ...
            if action_code in actions_with_committee:
                ...
                locs = [com_location_map[l] for l in action["Referral"].split("/") if l and l in com_location_map]
            ...
            if action_name == "passed House":
                actor = "lower"
            bills[bill_key].add_action(action_name, action_date, chamber=actor, classification=action_type)
    ...
    def scrape_documents(self, session, doc_type, chamber, bills, chamber_name=None):
        ...
        doc_path = "http://www.nmlegis.gov/Sessions/{}/{}/{}/".format(session_path, doc_type, chamber_name)
        ...
        for fname in doc.xpath("//a/text()")[1:]:
            ...
            media_type = "application/pdf" if fname.lower().endswith("pdf") else "text/html"

            # no suffix = just the bill
            if suffix == "":
                bill.add_version_link("introduced version", doc_path + fname, media_type=media_type)

            # floor amendments
            elif re.match(r"F(S|H)\d", suffix):
                a_chamber, num = re.match(r"F(S|H)(\d)", suffix).groups()
                a_chamber = "House" if a_chamber == "H" else "Senate"
                bill.add_document_link("{} Floor Amendment {}".format(a_chamber, num), doc_path + fname)
            # committee substitutes
            elif suffix.endswith("S"):
                committee_name = suffix[:-1]
                bill.add_version_link(
                    "{} substitute".format(committee_name), doc_path + fname, media_type=media_type,
                )
            ...
            # committee reports
                ...
                bill.add_document_link(
                    "{} committee report".format(committee_name), doc_path + fname, media_type=media_type,
                )
