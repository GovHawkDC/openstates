...
class WAPersonScraper(Scraper, LXMLMixin):
    ...
    def scrape_chamber(self, index_url, chamber):
        ...
        for member in doc.xpath('//div[@id="allMembers"]/div[@class="memberInformation"]'):
            ...
            leg = Person(name=name, primary_org=chamber, image=photo_url, district=district_num, party=party,)
            ...
            if capitol_address:
                leg.add_contact_detail(type="address", value=capitol_address, note="Capitol Office")
            if email:
                leg.add_contact_detail(type="email", value=email, note="Capitol Office")
            if capitol_phone:
                leg.add_contact_detail(type="voice", value=capitol_phone, note="Capitol Office")
            if capitol_fax:
                leg.add_contact_detail(type="fax", value=str(capitol_fax), note="Capitol Office")
            ...
            if _has_district_office:
                ...
                if district_phone:
                    leg.add_contact_detail(type="voice", value=district_phone, note="District Office")

                if district_address:
                    leg.add_contact_detail(type="address", value=district_address, note="District Office")
