...
rules = (
    ...
    Rule(["(?i)assigned to (?P<committees>.+?) Committee in"], ["referral-committee", "introduction"],),
    ...
    Rule([r"(?i)Amendment (?P<bills>.+?)\s+-\s+Introduced"], ["amendment-introduction"]),
    Rule([r"(?i)Amendment (?P<bills>[\w\s]+?) Passed"], ["amendment-passage"]),
    Rule([r"Amendment (?P<bills>.+?) -  Defeated by House of .+?\. Votes: Defeated"], ["amendment-failure"],),
