...
class TXCommitteeScraper(Scraper, LXMLMixin):
    ...
    def scrape_chamber(self, chamber):
        committee_list_urls = {
            "lower": "https://capitol.texas.gov/Committees/" "CommitteesMbrs.aspx?Chamber=H",
            "upper": "https://capitol.texas.gov/Committees/" "CommitteesMbrs.aspx?Chamber=S",
        }

        committee_list_url = committee_list_urls[chamber]
        committee_list_page = self.lxmlize(committee_list_url)

        committee_nodes = self.get_nodes(committee_list_page, '//form[@id="ctl00"]//a[@id="CmteList"]')
        ...
        for committee_node in committee_nodes:
            ...
            details_table = self.get_node(committee_page, '//div[@id="content"]//table[2]')
