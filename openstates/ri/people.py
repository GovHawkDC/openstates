...
class RIPersonScraper(Scraper, LXMLMixin):
    ...
    def scrape_chamber(self, chamber=None):
        ...
        for rownum in range(1, sh.nrows):
            d = {field: sh.cell(rownum, col_num).value for field, col_num in excel_mapping.items()}
            ...
            person = Person(
                primary_org=chamber, district=district, name=full_name, party=translate[d["party"]], image=photo_url,
            )
